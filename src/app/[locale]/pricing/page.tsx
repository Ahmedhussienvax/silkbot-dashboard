                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pt-12">
                        {[
                            ...
                            <div key={i} className={`p-10 glass-card border ${plan.border} relative group ${plan.popular ? 'scale-110 z-10 shadow-[0_0_80px_rgba(168,85,247,0.1)]' : ''}`}>
                                {plan.popular && (
                                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-accent-primary text-foreground text-[8px] font-black uppercase rounded-full tracking-[0.3em]">Dominant Flow</div>
                                )}
                                <div className={`w-14 h-14 rounded-2xl ${plan.bg} ${plan.color} flex items-center justify-center mb-8`}>
                                    <plan.icon className="w-7 h-7" />
                                </div>
                                <h3 className="text-2xl font-black text-foreground italic uppercase tracking-tighter">{plan.name}</h3>
                                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mt-1 mb-6">{plan.desc}</p>
                                <div className="text-5xl font-black text-foreground tracking-tighter mb-8">{plan.price}<span className="text-sm font-bold text-muted-foreground">/mo</span></div>
                                
                                <div className="space-y-4 mb-10">
                                    {plan.features.map((f, j) => (
                                        <div key={j} className="flex items-center gap-3 text-xs font-bold text-muted-foreground">
                                ...
                                <Link 
                                    ...
                                    className={`block w-full text-center py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all ${plan.popular ? 'bg-accent-primary text-foreground hover:scale-105 shadow-lg' : 'bg-surface border border-border text-muted-foreground hover:bg-foreground/10'}`}
                                >
