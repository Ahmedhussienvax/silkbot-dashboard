    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-500 font-arabic">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-border pb-10">
                ...
                <div className="flex items-center gap-6 bg-surface border border-border p-4 rounded-[2rem] backdrop-blur-3xl shadow-2xl">
                    <div className={cn(
                        "flex items-center gap-2 px-4 py-2 rounded-xl transition-all border",
                        config?.is_enabled ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : "bg-surface border-border text-muted-foreground"
                    )}>
            ...
            <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-12 gap-10 pb-32">
                ...
                <div className="lg:col-span-8 space-y-10">
                    ...
                    <section className="bg-gradient-to-br from-slate-900/60 to-black/60 backdrop-blur-2xl border border-border rounded-[3rem] p-10 shadow-3xl relative group overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="text-2xl font-black text-foreground tracking-tight italic uppercase">{t("prompt")}</h2>
                                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-[0.2em]">{t("prompt_desc")}</p>
                                </div>
                            </div>
                            <div className="flex gap-2">
                                <div className="px-4 py-2 bg-surface rounded-xl border border-border text-[9px] font-mono text-muted-foreground uppercase tracking-widest shadow-inner">
                                    UTF-8 Deployment
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative">
                            <textarea
                                ...
                                className="w-full bg-surface border border-border rounded-[2rem] px-10 py-8 text-foreground text-lg font-medium focus:ring-1 focus:ring-purple-500/30 outline-none leading-relaxed transition-all hover:bg-background/60 shadow-inner scrollbar-none"
                    ...
                    <section className="bg-gradient-to-br from-slate-900/60 to-black/60 backdrop-blur-2xl border border-border rounded-[3rem] p-10 shadow-3xl relative group overflow-hidden">
                        ...
                        <textarea
                            ...
                            className="w-full bg-surface border border-border rounded-[2rem] px-10 py-6 text-foreground text-base font-medium focus:ring-1 focus:ring-amber-500/30 outline-none leading-relaxed transition-all hover:bg-background/60 shadow-inner scrollbar-none"
                ...
                <div className="lg:col-span-4 space-y-10">
                    ...
                    <section className="bg-surface backdrop-blur-2xl border border-border rounded-[3rem] p-10 shadow-3xl space-y-10">
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                <div className="p-2 bg-amber-500/10 rounded-lg border border-amber-500/20">
                                    <Cpu className="w-5 h-5 text-amber-400" />
                                </div>
                                <h3 className="text-xl font-black text-foreground italic tracking-tight">{t("model")}</h3>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">{t("name")}</label>
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-surface border border-border rounded-2xl px-6 py-5 text-foreground font-bold focus:ring-1 ring-purple-500/50 outline-none transition-all shadow-inner" 
                                            ...
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    config?.ai_provider === provider.value
                                                        ...
                                                        : "bg-background/30 border-border text-muted-foreground hover:border-border hover:text-slate-300"
                                                )}
                                            >
                                                {provider.label}
                                            </button>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">{t("model_select")}</label>
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            className="w-full bg-surface border border-border rounded-2xl px-6 py-5 text-foreground font-black appearance-none outline-none focus:ring-1 ring-purple-500/50 transition-all cursor-pointer shadow-inner pr-12"
                                        >
                                            {availableModels.map((model) => (
                                                <option key={model.value} value={model.value} className="bg-background border-none">
                                                    {model.label}
                                                </option>
                                            ))}
                                        </select>
                        ...
                        <div className="space-y-10 pt-4 border-t border-border">
                            ...
                            <div className="space-y-5">
                                ...
                                <input type="range" min="0" max="1" step="0.1" value={config?.temperature || 0} onChange={(e) => setConfig(prev => prev ? { ...prev, temperature: parseFloat(e.target.value) } : null)} 
                                    className="w-full h-1 bg-surface rounded-lg appearance-none cursor-pointer accent-amber-500" />
                            </div>
                            ...
                            <div className="space-y-5">
                                ...
                                <input type="range" min="50" max="2000" step="50" value={config?.max_tokens || 500} onChange={(e) => setConfig(prev => prev ? { ...prev, max_tokens: parseInt(e.target.value) } : null)} 
                                    className="w-full h-1 bg-surface rounded-lg appearance-none cursor-pointer accent-cyan-500" />
                            </div>
                        </div>
                    </section>

                    {/* Save Button */}
                    <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/10 backdrop-blur-3xl border border-border rounded-[3rem] p-10 shadow-3xl text-center space-y-8 relative overflow-hidden group">
                         ...
                         <div className="p-5 bg-surface rounded-3xl inline-block shadow-inner border border-border relative z-10">
                            <ShieldCheck className="w-10 h-10 text-foreground opacity-80" />
                        </div>
                        <div className="relative z-10">
                            <h3 className="text-xl font-black text-foreground italic uppercase tracking-tighter">Synchronize Core</h3>
                            <p className="text-[11px] text-muted-foreground font-bold leading-relaxed px-4 opacity-70">
                        ...
                        <button
                            type="submit"
                            disabled={saving}
                            className="w-full h-20 bg-white text-black font-black text-[10px] tracking-[0.4em] uppercase rounded-[2rem] hover:bg-purple-500 hover:text-foreground transition-all transform hover:scale-[1.02] active:scale-95 shadow-2xl relative z-10 flex items-center justify-center gap-4"
                        >
