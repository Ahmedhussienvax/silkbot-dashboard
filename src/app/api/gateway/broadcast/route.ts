...
export async function POST(req: NextRequest) {
  try {
    const { instance, targets, message } = await req.json();
    if (!instance || !Array.isArray(targets) || !message) {
      return NextResponse.json(
        { error: "Missing required fields: instance, targets, message" },
        { status: 400 }
      );
    }
    ...
    const response = await fetch(`${gatewayUrl}/instance/broadcast`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-api-key": gatewayApiKey,
      },
      body: JSON.stringify({ instance, targets, message }),
      cache: "no-store",
    });
