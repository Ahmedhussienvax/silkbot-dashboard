...
import { Menu, X, Zap, LayoutDashboard, MessageSquare, Send, Users, Settings, LogOut, Shield, Columns, Brain, BookOpen, Activity, CreditCard } from "lucide-react";
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 h-20 bg-background border-b border-glass-border px-6 flex items-center justify-between z-[100] shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className="w-10 h-10 rounded-xl bg-surface flex items-center justify-center border border-glass-border active:scale-95 transition-all text-[10px] font-black text-foreground"
                    >
                        {locale === 'en' ? 'AR' : 'EN'}
                    </button>
                    <button 
                        ...
                        className="w-10 h-10 rounded-xl bg-surface flex items-center justify-center border border-glass-border active:scale-95 transition-all"
                    >
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 left-0 bottom-0 w-[85%] max-w-sm bg-background border-r border-border z-[105] shadow-2xl flex flex-col"
                        >
                            <div className="relative z-10 space-y-12 p-8 flex-grow overflow-y-auto">
                                        ...
                                        <Link 
                                            ...
                                            className={cn(
                                                ...
                                                activeItem === item.id 
                                                    ...
                                                    : "bg-surface border border-glass-border hover:bg-foreground/[0.08]"
                                            )}
                                        >
                                            <item.icon className={cn(
                                                "w-6 h-6 transition-colors",
                                                activeItem === item.id ? "text-accent-primary" : "text-muted-foreground"
                                            )} />
                                            <span className={cn(
                            ...
                            <div className="space-y-6 pt-12">
                                {(!isGlobalAdmin && isOwner) && (
                                    <Link 
                                        href="/pricing"
                                        onClick={() => setIsOpen(false)}
                                        className="block p-8 rounded-[2rem] bg-gradient-to-br from-accent-primary to-accent-secondary border border-border shadow-2xl relative overflow-hidden group/upgrade transition-transform active:scale-95"
                                    >
                                        <Zap className="absolute -right-4 -bottom-4 w-24 h-24 text-foreground/10 group-hover:scale-110 transition-transform duration-700" />
                                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1 italic">SilkBot Pro Integration</p>
                                        <p className="text-foreground font-black text-lg leading-tight uppercase tracking-tighter italic">Upgrade to<br />Enterprise Hub</p>
                                        <div className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-foreground uppercase tracking-widest bg-background/20 px-4 py-2 rounded-xl">
                                ...
                                <button 
                                    ...
                                    className="w-full flex items-center gap-5 p-5 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-foreground transition-all group active:scale-95"
                                >
                                    <LogOut className="w-6 h-6 text-red-500 group-hover:text-foreground" />
